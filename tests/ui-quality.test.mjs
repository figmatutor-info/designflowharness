...
import {
  ...
  checkActions,
  previewErrors,
} from "../scripts/lib/screen-contract.mjs";
...
import {
  ...
  fileHash,
  AUDIT_KEYS,
} from "../scripts/lib/design-evidence.mjs";

function contractFor(frames) {
  return {
    schema_version: 1,
    screens: frames.map((frame, i) => ({
      id: `screen-${i}`,
      purpose: "자료 탐색",
      references: ["analysis.md · 카드 패턴"],
      states: [
        {
          id: "default",
          required: true,
          ...
          action: { mode: "collection", id: "open-item" },
          checks: ["필터 선택이 보임"],
        },
      ],
    })),
...
test("행동 계약: 동등한 카드 여러 개 허용, 이름만 primary로 바꾸면 실패", () => {
  const frame = {
    name: "Library",
    nodes: [
      { id: "1", actionId: "open-item", isTapTarget: true },
      { id: "2", actionId: "open-item", isTapTarget: true },
    ],
  };
...
test("상태별 single/none 정책과 필수 상태 누락·중복을 검사", () => {
  const frame = {
    name: "Submit",
    nodes: [{ actionId: "open-item", isTapTarget: true }],
  };
  const contract = contractFor([frame]);
...
test("HTML: 화면 수만 맞추거나 주석으로 상태 마커를 넣어 통과할 수 없음", () => {
  ...
  state.id = "empty";
  state.frame = "Home empty";
  state.screenshot = "design/04-screens/screenshots/empty.png";
  contract.screens[0].states.push(state);
  const defaultHtml =
    '<section data-screen="screen-0" data-state="default"></section>';
  const emptyHtml =
    '<section data-state="empty" data-screen="screen-0"></section>';
  assert.equal(previewErrors(defaultHtml + emptyHtml, contract).length, 0);
  assert.equal(
    previewErrors(defaultHtml + `<!-- ${emptyHtml} -->`, contract).length,
    1,
  );
  assert.equal(
    previewErrors(defaultHtml + `<script>${emptyHtml}</script>`, contract)
      .length,
    1,
  );
  assert.equal(
    previewErrors(defaultHtml + emptyHtml + emptyHtml, contract).length,
    1,
  );
});
...
test("실제 check-snapshot/audit은 계약을 공유하고 구형 isPrimary를 강제하지 않음", () => {
  ...
  const schema = run("check-snapshot.mjs", [
    "--snapshot",
    snapPath,
    "--file-key",
    fixture("figma-file-key.txt"),
    "--contract",
    contractPath,
  ]);
  ...
  const args = [
    ...
    snapPath,
    "--rules",
    fixture("design-rules.md"),
    "--contract",
    contractPath,
    "--output",
    path.join(tmpDir(), "contract-audit.json"),
    "--json",
  ];
...
test("Figma lint와 로컬 overflow 검사가 같은 스크롤 동작을 판정", () => {
  ...
  const helper = source.slice(
    source.indexOf("function isScrollViewport("),
    source.indexOf("async function isExempt("),
  );
  const check = source.slice(
    source.indexOf("function checkOverflow("),
    source.indexOf("function checkText("),
  );
  const lint = new Function(
    "report",
    `const TOLERANCE = 1; ${helper}\n${check}\nreturn checkOverflow;`,
  );
...
test("스크롤 안의 화면 밖 텍스트는 safe-area 위반 아님; viewport 자체 침범은 실패", () => {
  ...
  const viewport = {
    ...
    type: "FRAME",
    position: { x: 16, y: 100 },
    size: { width: 358, height: 600 },
    clipsContent: true,
    overflowDirection: "VERTICAL",
    layout: { layoutMode: "VERTICAL", vSizing: "FIXED" },
  };
  frame.nodes.push(viewport, {
    id: "offscreen-text",
    name: "Last item",
    parentId: "viewport",
    type: "TEXT",
  ...
  const args = [
    ...
    fixture("design-rules.md"),
    "--output",
    path.join(tmpDir(), "scroll-audit.json"),
    "--json",
  ];
...
function evidenceProject(t) {
  ...
  for (const file of [
    ...
    "design/03-design-rules/design-direction.md",
    "design/03-design-rules/preview.html",
  ])
    put(file, "reviewed design");
  put(
    "design/03-design-rules/design-rules.md",
    "status: confirmed\nimage-slots: none\n",
  );
  ...
  const snapshot = {
    ...
    file_key: "test-key",
    pages: ["01 Tokens", "02 Components", "03 Screens"].map((name) => ({
      name,
  ...
  const seal = exec("--seal");
  assert.equal(seal.code, 0, seal.out);
  const manifest = read(EVIDENCE_PATH);
  const review = draftReview(manifest, contract);
  for (const view of review.views) {
    view.comparison =
      "승인 시안과 같은 폭이며 카드 제목과 필터의 위계가 유지됨";
    for (const score of Object.values(view.scores)) {
      score.score = 4;
      score.evidence = "Library 상단 제목과 카드 문구가 겹침 없이 읽힘";
    }
    for (const check of Object.values(view.checks)) {
      check.passed = true;
      check.evidence = "Library 화면에서 조건 확인";
    }
  }
...
test("캡처 후 snapshot_date가 같아도 내용/스크린샷/규칙 변경은 이전 PASS 만료", (t) => {
  const p = evidenceProject(t);
  p.snapshot.extra = "changed";
  p.put(SNAPSHOT_PATH, p.snapshot);
  assert.match(p.exec("--check").out, /스냅샷\/스크린샷 변경/);
  delete p.snapshot.extra;
  p.put(SNAPSHOT_PATH, p.snapshot);
  p.put(
    p.pngPath,
    Buffer.concat([
      readFileSync(path.join(p.root, p.pngPath)),
      Buffer.from("changed"),
    ]),
  );
...
test("merge: 다른 캡처 배치 혼합 거부, 같은 캡처는 page 메타 보존", () => {
  const batch = (from, id) => ({
    ...
    file_key: "test",
    snapshot_date: "2026-09-20",
    frame_range: { from, to: from + 1, total_frames: 2 },
    variables: {},
    page: {
      name: "03 Screens",
      profile: "full",
      capture_id: id,
      frames: [{ name: `Screen ${from}`, nodes: [] }],
    },
  });
  ...
  assert.match(
    run("merge-snapshot.mjs", [a, b, "--out", out]).out,
    /capture_id 불일치/,
  );
...
test("최종 게이트: 로그 제목 대신 상태를 확인, 날짜가 같아도 오래된 audit 거부", (t) => {
  ...
  p.put(
    "design/04-screens/audit-report.md",
    "PASS라고 쓴 문서만으로 통과할 수 없음",
  );
  ...
  for (let i = 1; i < 5; i++)
    p.put(`design/04-screens/screenshots/screen-${i}.png`, png);
  const audit = {
    passed: true,
    snapshot_date: p.snapshot.snapshot_date,
    input_hashes: {
      snapshot: fileHash(path.join(p.root, SNAPSHOT_PATH)),
      rules: fileHash(
        path.join(p.root, "design/03-design-rules/design-rules.md"),
      ),
      contract: fileHash(path.join(p.root, contractPath)),
    },
    results: Object.fromEntries(
      AUDIT_KEYS.map((key) => [key, { status: "PASS" }]),
    ),
  ...
  const gate = () => {
    const result = spawnSync(
      process.execPath,
      [
        path.join(ROOT, "scripts/check-phase.mjs"),
        "--phase",
        "screens",
        "--json",
        "--shallow",
