...
이미지를 쓰지 않는 앱이면 (가계부·설정·계산기 등) 이렇게 적는다:
...
`none` 이면 STAGE=assets 는 통째로 건너뛰고, 게이트 4의 이미지 검사도
"해당 없음"으로 통과한다. 아래 항목들은 적지 않아도 된다.

**판단 기준:** screens.md 의 어느 화면에도 사진·일러스트 자리가 없으면 `none`.
아이콘은 이미지가 아니다 (컴포넌트로 만든다). 애매하면 `used` 로 두고
슬롯 계획을 비워두지 말고 최소 1개는 적는다.
