...
| 값     | 뜻                   | 결과                                                         |
| ------ | -------------------- | ------------------------------------------------------------ |
| `used` | 이미지를 쓴다 (기본) | screens 가 §I 표대로 슬롯을 채움 · 게이트 4가 빈 슬롯을 검사 |
| `none` | 이미지를 안 쓴다     | `Img/` 슬롯을 만들지 않음 · 게이트 4 이미지 검사 "해당 없음" |

`none` 이면 아래 항목들(라이브러리·슬롯 계획 등)은 적지 않아도 된다.
...
`image-slots` 줄 바로 아래에 적는다. 생략하면 기본값 `design/assets/characters`.

    image-library: design/assets/characters

**규칙을 쓰기 전에 이 폴더를 `Glob` 으로 훑어 실제 파일 목록을 손에 쥔다.**
아래 슬롯 표의 `파일` 열에는 여기 있는 파일명만 적을 수 있다. 없는 파일을 적으면
check-assets 가 FAIL 하고, 필요한 이미지가 없으면 만들지 말고 사용자에게 폴더에 넣어 달라고 요청한다.

### 파일 선택 기준

{한 문단. 사용자가 기준을 정했으면 그 기준으로, 안 정했으면
scripts/default-tokens.md §I 의 "파일 선택 기준 (기본)" 을 그대로 쓰고 가정 로그에 남긴다}
...
| ------------ | ------- | ---- | ------ | ----------------- | ----------------- |
| 01-home-hero | 01-home | hero | `16:9` | `buddy-front.png` | {무엇을 보여줄지} |
| ...          | ...     | ...  | ...    | ...               | ...               |
...
- Library: lucide
- Icons: home / book-open / target / archive / star / search / chevron-right / bell
  (screens.md 에 나오는 아이콘을 **lucide 공식 이름**(kebab-case)으로 전부 나열한다.
  figma-builder 는 이 이름을 CDN 에서 받아 만든다 — 손으로 그리지 않는다.
  이름은 https://lucide.dev/icons 에서 확인. 없는 이름을 적으면 builder 가 멈춘다)
