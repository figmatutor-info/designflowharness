...
export function projectFile(root, path) {
  ...
  if (!path || isAbsolute(path) || rel.startsWith("..") || isAbsolute(rel))
    throw new Error(`프로젝트 밖 경로: ${path}`);
  return full;
}
...
export function inputHashes(root) {
  const paths = [
    ...
    "design/02-structure/screen-contract.json",
  ...
  const rules = readFileSync(
    projectFile(root, "design/03-design-rules/design-rules.md"),
    "utf8",
  );
  if (!/^\s*image-slots:\s*none\s*$/m.test(rules)) {
    const library =
      rules.match(/^\s*image-library:\s*(.+?)\s*$/m)?.[1] ||
      "design/assets/characters";
    const walk = (dir) => {
      for (const item of readdirSync(projectFile(root, dir), {
        withFileTypes: true,
      })) {
  ...
  return Object.fromEntries(
    paths.sort().map((path) => [path, fileHash(projectFile(root, path))]),
  );
}
...
export function captureArtifacts(root, contract, startedAt) {
  return Object.fromEntries(
    requiredViews(contract).map((view) => {
      ...
      if (
        ...
        throw new Error(`PNG 아님: ${view.screenshot}`);
      if (statSync(path).mtimeMs < Date.parse(startedAt))
        throw new Error(
          `캡처 시작 전 스크린샷: ${view.screenshot} — Figma에서 다시 내보낼 것`,
        );
...
export function reviewErrors(review, manifest, contract) {
  ...
  if (
    ...
    review?.capture_id !== manifest.capture_id ||
    review?.manifest_sha256 !== hash(JSON.stringify(manifest))
  )
    errors.push("시각 검수 버전/캡처/manifest 해시 불일치");
  const rows = Array.isArray(review?.views) ? review.views : [];
  const views = requiredViews(contract);
  if (rows.length !== views.length)
    errors.push("시각 검수 화면·상태 수 불일치");
  for (const view of views) {
    const matches = rows.filter((r) => r.frame === view.frame);
    if (matches.length !== 1) {
      errors.push(`${view.frame}: 시각 검수 누락/중복`);
      continue;
    }
    const row = matches[0];
    if (row.screenshot_sha256 !== manifest.screenshots[view.screenshot])
      errors.push(`${view.frame}: 검수 스크린샷 해시 불일치`);
    if (!text(row.comparison))
      errors.push(`${view.frame}: 승인 시안 대비 관찰 근거 필요`);
    ...
    if (!Array.isArray(row.issues))
      ...
      for (const issue of row.issues) {
        if (
          ...
          errors.push(`${view.frame}: 결함 근거/수정 또는 수용 사유 필요`);
        if (
          ["critical", "major"].includes(issue?.severity) &&
          issue.resolved !== true
        )
          errors.push(`${view.frame}: 미해결 ${issue.severity}`);
      }
  }
  return errors;
}
...
export function draftReview(manifest, contract) {
  return {
    ...
    views: requiredViews(contract).map((view) => ({
      frame: view.frame,
      screenshot_sha256: manifest.screenshots[view.screenshot],
      comparison: "",
      scores: Object.fromEntries(
        DIMENSIONS.map((key) => [key, { score: null, evidence: "" }]),
      ),
      checks: Object.fromEntries(
        [...VISUAL_CHECKS, ...view.checks].map((key) => [
          key,
          { passed: null, evidence: "" },
        ]),
      ),
      issues: [],
    })),
